...
```
* 二分查找target - n其实也可以优化一下，如果第一个数确定了就能锁定出第二个数的值，然后利用二分查找在数组里面查找是否存在这个数。当然这需要传入数组是一个有序的数组，但是考虑到这种算法的时间复杂度为O(nlogn)，所以可以利用快排函数qsort先进行排序，再进行二分查找
### ！方法二：哈希表
## ！53.最大子数组和  
###  方法一：动态规划  
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMzg2NjQ5NTYxLC02NzE3Mjk2MjgsMjE4Mj
